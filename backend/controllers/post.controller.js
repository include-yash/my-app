...
export const getAllPosts = async(req, res)=>{
    try {
        ...
        res.status(500).json({message: "Error in getAll Posts", error})
...
export const getLikedPosts = async(req, res)=>{
    ...
    try {
        ...
        res.status(500).json({message: "Error in getLikedPosts", error})
...
export const getFollowingPosts = async(req, res)=>{
    ...
    res.status(200).json(feedPosts)
        ...
        res.status(500).json({message: "Error in getFollowingPosts", error})
    }
}
...
export const getUserPost = async(req, res)=>{
    try{
        ...
        const posts = await Post.find({user: user._id}).sort({created: -1})
        .populate({
            path: "user",
            select: "-password",
        })
        ...
        res.status(500).json({message: "Error in getUserPost", error})
    }
}